...
export default async function handler(req:NextApiRequest, res:NextApiResponse) {
    if (req.method === 'POST') {
      try {
        // Assuming the file is sent as form data with the key 'image'
        const { image } = req.body;
  
        // Handle the file, save it, etc.
        // You can use libraries like `multer` for handling file uploads.
  
        // For simplicity, let's just log the file details
        console.log('Uploaded image:', image);
        // var gg = await quickstart(image);
    // console.log("image wala hi")
    // console.log(gg);
